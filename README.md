...
**中文优先的 OpenClaw Skill 安全验证器。GUI 是主入口，CLI 适合自动化和高级用户。**

OpenClaw Skill Guard 用于在安装、发布或审查 OpenClaw Skill 之前做本地安全验证。它会分析 `SKILL.md`、skill 目录、skills 根目录，也支持直接粘贴 HTTPS skill 链接进行扫描。报告会解释安装链、权限/能力声明、环境变量、外部链接、提示词、间接指令、攻击路径、运行时约束和组合风险。
...
4. 如果需要自动化扫描，同一个压缩包内也提供 `openclaw-skill-guard.exe` CLI。
...
```powershell
.\target\release\openclaw-skill-guard.exe scan https://github.com/example/example-skill --format html
```

策略与 CI：

```powershell
.\target\release\openclaw-skill-guard.exe scan <本地路径或HTTPS链接> --config .\.openclaw-guard.yml --ci
.\target\release\openclaw-skill-guard.exe scan <本地路径或HTTPS链接> --no-network
.\target\release\openclaw-skill-guard.exe scan <本地路径或HTTPS链接> --agent-ecosystem
```
...
OpenClaw Skill Guard is a Chinese-first, Windows-friendly Rust verifier for reviewing OpenClaw Skills before installation, release, or audit. The GUI is the primary product surface; the CLI is intended for automation and advanced workflows.
