...
pub fn load_runtime_manifest(
    ...
    let mut manifest = RuntimeManifest {
        ...
        permission_surface: default_permission_surface(),
        permission_schema: default_permission_schema(),
    ...
    let safe_checks = infer_safe_local_facts(
        target_path,
        &manifest.expected_env_vars,
        &manifest.expected_config_files,
    );
    ...
    manifest.permission_schema = permission_schema_from_surface(&manifest.permission_surface);
...
fn parse_manifest_content(
    path: &Path,
    content: &str,
) -> Result<RawRuntimeManifest, RuntimeManifestError> {
    let extension = path
        .extension()
...
fn from_raw_manifest(raw: RawRuntimeManifest) -> RuntimeManifest {
    ...
    let network = normalize_availability(permission.network.clone());
    let writable_scope = normalize_writable_scope(permission.writable_scope.as_deref());
    let exec_allowed = normalize_availability(permission.exec_allowed.clone());
    let process_allowed = normalize_availability(permission.process_allowed.clone());
    let browser_available = normalize_availability(permission.browser_available.clone());
    let web_fetch_available = normalize_availability(permission.web_fetch_available.clone());
    let web_search_available = normalize_availability(permission.web_search_available.clone());
    let gateway_available = normalize_availability(permission.gateway_available.clone());
    let nodes_available = normalize_availability(permission.nodes_available.clone());
    let cron_available = normalize_availability(permission.cron_available.clone());
    let root_admin_hint = normalize_availability(permission.root_admin_hint.clone());
    let home_directory_access = normalize_availability(permission.home_directory_access.clone());
    let permission_surface = PermissionSurface {
        ...
        browser_network: browser_available,
        root_admin_hint,
    ...
    RuntimeManifest {
        ...
        permission_schema: permission_schema_from_surface(&permission_surface),
        permission_surface,
...
fn default_permission_surface() -> PermissionSurface {
    PermissionSurface {
...
fn default_permission_schema() -> PermissionSchema {
    permission_schema_from_surface(&default_permission_surface())
}

fn permission_schema_from_surface(surface: &PermissionSurface) -> PermissionSchema {
    PermissionSchema {
        schema_version: "v1".to_string(),
        capability_surface: CapabilitySurface {
            exec_allowed: surface.exec_allowed,
            process_allowed: surface.process_allowed,
            shell_allowed: surface.shell_allowed,
            child_process_allowed: surface.child_process_allowed,
            write_allowed: surface.write_allowed,
            edit_allowed: surface.edit_allowed,
            apply_patch_allowed: surface.apply_patch_allowed,
            direct_network: surface.direct_network,
            browser_network: surface.browser_network,
            web_fetch: surface.web_fetch_available,
            gateway: surface.gateway_available,
            nodes: surface.nodes_available,
            cron: surface.cron_available,
            ...
            local_secret_paths_available: surface.home_directory_access,
            browser_store_proximity: AvailabilityState::Unknown,
        },
        environment_scope: EnvironmentScope {
            workspace_only: if surface.writable_scope == WritableFileSystemScope::WorkspaceOnly {
                AvailabilityState::Enabled
            } else {
                AvailabilityState::Unknown
            },
            home_access: surface.home_directory_access,
            mounted_paths: surface.mounted_directories.clone(),
            mounted_secrets: surface.mounted_secrets_or_configs.clone(),
            writable_scope: surface.writable_scope,
            read_only_scope: if surface.writable_scope == WritableFileSystemScope::ReadOnly {
                AvailabilityState::Enabled
            } else {
                AvailabilityState::Unknown
            },
        },
        privilege_hint: match surface.root_admin_hint {
            AvailabilityState::Enabled => PrivilegeHint::RootAdmin,
            AvailabilityState::Disabled => PrivilegeHint::StandardUser,
            AvailabilityState::Unknown => PrivilegeHint::Unknown,
        },
    }
}
...
fn facts_from_manifest(manifest: &RuntimeManifest) -> Vec<RuntimeFact> {
    ...
    for (key, value) in [
        ...
        (
            "browser_available",
            manifest.permission_surface.browser_available,
        ),
        (
            "web_fetch_available",
            manifest.permission_surface.web_fetch_available,
        ),
        (
            "gateway_available",
            manifest.permission_surface.gateway_available,
        ),
        (
            "nodes_available",
            manifest.permission_surface.nodes_available,
        ),
...
fn normalize_availability(value: Option<RawAvailability>) -> AvailabilityState {
    match value {
        ...
        Some(RawAvailability::StringValue(value)) => {
...
fn normalize_writable_scope(value: Option<&str>) -> WritableFileSystemScope {
    match value.map(|value| value.trim().to_ascii_lowercase()) {
        Some(value) if value == "read_only" || value == "readonly" => {
            WritableFileSystemScope::ReadOnly
        }
        Some(value) if value == "workspace_only" || value == "workspace" => {
            WritableFileSystemScope::WorkspaceOnly
        }
        Some(value) if value == "home_directory" || value == "home" => {
            WritableFileSystemScope::HomeDirectory
        }
        Some(value) if value == "user_files" || value == "user" => {
            WritableFileSystemScope::UserFiles
        }
...
mod tests {
    ...
    fn parses_minimal_json_manifest() {
        ...
        assert_eq!(
            result.manifest.execution_environment,
            RuntimeEnvironment::Sandbox
        );
        assert_eq!(
            result.manifest.permission_surface.network,
    ...
    fn parses_partial_yaml_manifest_and_tolerates_unknown_fields() {
        ...
        assert_eq!(
            result.manifest.permission_surface.exec_allowed,
            AvailabilityState::Enabled
        );
    }
